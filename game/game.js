...
// note: 
// run the local game at 16ms/ 60hz
// window.global on server we run at 45ms, 22hz
const frameTime = window.global ? 45 : (60/1000);

// server shim frame animation
( function () {
	...
	var gameCore = function(gameInstance){
		...
		if(this.server) {
		   ...
		   this.players.self.pos = {x:20,y:20};
			...
			this.ghosts = {
				...
				serverPosOther : new gamePlayer(this),
					//The other players ghost destination position (the lerp)
				posOther : new gamePlayer(this)
			};
		...
		if(!this.server) {
			...
			//todo: Make this only if requested
...
//server side we set the 'gameCore' class to a global type, so that it can use it anywhere.
...
gameCore.prototype.vAdd = function(a,b) { return { x:(a.x+b.x).fixed(), y:(a.y+b.y).fixed() }; };
	//Subtract a 2d vector with another one and return the resulting vector
gameCore.prototype.vSub = function(a,b) { return { x:(a.x-b.x).fixed(),y:(a.y-b.y).fixed() }; };
	//Multiply a 2d vector with a scalar value and return the resulting vector
gameCore.prototype.vMulScalar = function(a,b) { return {x: (a.x*b).fixed() , y:(a.y*b).fixed() }; };
	//For the server, we need to cancel the setTimeout that the polyfill creates
gameCore.prototype.stopUpdate = function() {  window.cancelAnimationFrame( this.updateid );  };
...
gameCore.prototype.vLerp = function(v,tv,t) { return { x: this.lerp(v.x, tv.x, t), y:this.lerp(v.y, tv.y, t) }; };
...
}; //gameCore.physicsMovementVectorFromDirection
...
}; //gameCore.prototype.updatePhysics
...
}; //gameCore.clientProcessNetPredictionCorrection
...
}; //gameCore.clientProcessNetUpdates

gameCore.prototype.clientOnserverupdateRecieved = function(data){
...
}; //gameCore.clientOnserverupdateRecieved
...
}; //gameCore.prototype.clientUpdateLocalPosition
...
}; //gameCore.clientCreatePhysicsSimulation
...
}; //gameCore.clientCreatePingTimer
...
};//gameCore.clientCreateConfiguration
...
gameCore.prototype.clientCreateDebugGui = function() {
	...
	var _netsettings = this.gui.addFolder('Networking');
		...
		_netsettings.add(this, 'netOffset').min(0.01).step(0.001).listen();
		_netsettings.add(this, 'serverTime').step(0.001).listen();
		_netsettings.add(this, 'clientTime').step(0.001).listen();
		//_netsettings.add(this, 'oldestTick').step(0.001).listen();

		_netsettings.open();

}; //gameCore.clientCreateDebugGui
...
}; //gameCore.clientOnOtherclientcolorchange
...
}; //gameCore.clientConnectToServer
